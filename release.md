...
## 3.0.0
**摘要：重大 API 重构与行为变更。** 移除了 `data_count(int fixed_count)`、 `line_width(int fixed_count)`、`cliargs_parse_by_format` 等函数；将 `cliargs_parse_by_parser` 更名为 `cliargs_parse_custom`，并删除 `name` 参数；更新了 `data_count(int, int)` 与 `line_width(int, int)` 的函数签名。

**破坏性变更 (Breaking Changes)**
* **API 移除**：
  * 彻底移除了 `data_count(int fixed_count)` 和 `line_width(int fixed_count)` 函数。
  * 彻底移除了 `cliargs_parse_by_format` 函数。
  * 彻底移除了 `cliargs_parse_by_format` 函数，替换为 `cliargs_parse_custom` 函数。
* **签名变更**：`data_count(int at_least, int at_most)` 函数已替换为 `data_count(int at_least, int at_most = -1)`。虽然增加了默认值以提供便利，但这改变了函数签名。依赖旧签名的函数指针或严格的模板推导的现有代码将会中断。
* **签名变更**：`line_width(int at_least, int at_most)` 函数已替换为 `line_width(int at_least, int at_most = -1)`。虽然增加了默认值以提供便利，但这改变了函数签名。依赖旧签名的函数指针或严格的模板推导的现有代码将会中断。
...
## 2.0.0
**摘要：重大 API 重构与行为变更。** `error()` 和 `print_help()` 函数已从 `Parser` 移至 `Result`。默认解析模式已从 GNU 模式切换为 Smart 模式 (`sensitive_mode`)。标量类型参数允许被重复指定，最后一次指定的值被采纳。

**破坏性变更 (Breaking Changes)**
* **API 迁移**：`error()` 和 `print_help()` 函数已从 `Parser` 类移至 `Result` 类。现在应调用 `Result::error()` 和 `Result::print_help()`。
* **Parser 接口更新**：
...
* **模式重命名**：`Parser::sensitive_mode()` 函数已被 `Parser::gnu_mode()` 取代。
